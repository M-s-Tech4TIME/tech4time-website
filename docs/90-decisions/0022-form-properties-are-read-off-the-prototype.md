...
**That is what happened.** `tech4time-website-backend/sections/careers.php` has carried
...
`tech4time-website-backend/public/assets/js/admin-forms.js` began posting the editors with `fetch(form.action || location.href)`. From that
...
public contact page and on `tech4time-website-backend/public/setup.php`, `id` and `title` are
already controls on the careers screen, and `method` and `reset` are one ordinary field name away from breaking the contact form's
...
Every existing suite passed throughout. `tech4time-website-backend/tools/test_admin_forms.py` ran 215 checks over these
screens at the time and `tech4time-website-backend/tools/check_admin_a11y.py` ran 1845, and neither could see it: both drive the markup,
...
under an unhelpful name. So `tech4time-website-backend/tools/test_admin_forms.py` also asks the module, on every admin
...
- Two lines of `tech4time-website-backend/public/assets/js/admin-forms.js` and three of `assets/js/forms.js` go through a helper. Both helpers carry the
  reasoning in full, because it is not deducible from the code.
- `tech4time-website-backend/public/assets/js/admin-forms.js` exposes `postUrl` on `window.Tech4Time.adminForms` so the browser suite can
