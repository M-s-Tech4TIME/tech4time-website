...
`assets/js/analytics.js` is the same two lines in a file this host serves, reading the id from a
`data-ga` attribute on its own tag.

**Two policies, and the strict one is the one that decides.** `.htaccess` names the Google origins
unconditionally, because a header cannot read a JSON file — that is what makes the field *able* to
work without a deploy. What actually keeps them shut is the `<meta>` policy `lib/head.php` sends
...
- **`tools/audit_pages.py` asserts it.** With no id, *any* external origin in a rendered page is a
...
**Widen only `.htaccess` and leave the meta policy alone.** Then the page-level policy would refuse
