...
`upload_accept()` stored every picture at one 1600px ceiling while `tools/build_images.py` had
...
- **Deleted:** `--logo-src` in `theme.css`, declared three times and consumed nowhere;
...
- **`tools/build_logos.py` and `tools/build_favicons.py` are KEPT**, against the original plan,
  which said the server does this now. It does not do *their* job: they build the committed **seed**
  from the master artwork in `tools/masters/`, and the server makes renditions of an **upload** into
...
- **`tech4time-website-backend/tools/test_end_to_end.py` exists because of this work.** Each half is tested against an
...
- **`/favicon.ico` and `/assets/css/brand.css` are two more generated files served at static-looking
  ...
  the document is missing; `brand.css` with the shipped palette is an **empty file**.
