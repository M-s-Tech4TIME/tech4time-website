...
**3. The `<head>` is emitted once, by `lib/head.php`, not pasted into every page.**
...
`pages/contact/index.php` rendered them from `content/contact.json`. Editing an office in the admin
left sixteen pages advertising the old one — and `tools/sync_site_contact.py` existed to paste the
...
`tech4time-website-backend/sections/seo.php` iterates the `meta` band alone.
`tech4time-website-backend/tools/test_seo_admin.py` opens each page editor, saves it without
...
- **`sitemap.xml`, `robots.txt` and `site.webmanifest` are rendered**, by `sitemap.php`,
  `robots.php` and `manifest.php`, reached by internal rewrites so no address changes. The static
  files are gone.
...
- **`lastmod` is each document's publish stamp, or absent.** The ten dates typed into `sitemap.php`
...
- **The crawl controls are otherwise unguarded**, behind a confirmation that names the consequence,
  ...
  weigh up, it is the off switch, and `robots.php` writes `Allow: /` above it either way.
- **`404.html` became `404.php`**, so the site has no static page left. It has no canonical and no
  `og:url`: it is served at every address that does not exist and has none of its own.
- **`tools/sync_site_contact.py` lost its JSON-LD half** and keeps only the footer block, which is
