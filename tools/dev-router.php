...
/* The uploads allow-list, and the headers that make a vector file safe.
   .htaccess section 8 is the authoritative copy; this is the same rule for
   the machine the page is actually being written on. Without it the built-in
   server hands over ANY name under uploads/ as a static file, which is the
   one directory on this site holding bytes that arrived over the network —
   and it would serve an .svg as a document, which the host deliberately does
   not. A route that behaves differently in only one of the two places is the
   fault this router exists to prevent. */
